...
public class Warehouse {
	...
	public void arrive(long itemId, int amount) {
		...
		log.info("item arrived - item:{} amount:{}", itemId, currentAmount + " -> " + newAmount);
	}
	...
	public void ship(long id, int amount) {
		...
		if (currentAmount < amount) {
			log.error("item out of stock - current amount:{} / order amount:{}", currentAmount, amount);
			return;
		}
		...
		log.info("item shipped - item:{} amount:{}", id, currentAmount + " -> " + newAmount);
	}
